...
export default function RunwayFYIAdminLooks() {
  ...
  const handleSaveEdit = async (lookId: number) => {
    ...
    setSaving(lookId)
    try {
      const res = await fetch(`${RAILWAY_API}/api/trends/shows/${selectedShow!.id}/cover-image`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ image_url: editUrl.trim() }),
      })
      // Use cover-image endpoint for look 1, otherwise update via direct look endpoint
      // Actually update the specific look's image_url
      const updateRes = await fetch(`${RAILWAY_API}/api/trends/shows/${selectedShow!.id}/looks/${lookId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ image_url: editUrl.trim() }),
      })
      if (updateRes.ok || res.ok) {
  ...
  const handleDelete = async (lookId: number, showId: number) => {
    if (!confirm('Delete this look?')) return
    setDeleting(lookId)
    try {
      const res = await fetch(`${RAILWAY_API}/api/trends/shows/${showId}/looks/${lookId}`, {
        method: 'DELETE',
      })
      if (res.ok) {
    ...
    <>
      ...
      {message && (
        <div style={{
          ...
          background: message.type === 'ok' ? '#1a7a4a' : '#c0392b',
          color: '#fff', fontFamily: 'var(--f-mono)', fontSize: 11,
      ...
      <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
        ...
        <div style={{
          width: 280, flexShrink: 0, background: 'var(--ink)', color: '#fff',
          display: 'flex', flexDirection: 'column', overflow: 'hidden',
        }}>
          {/* Sidebar header */}
          <div style={{ padding: '20px 20px 12px' }}>
            <p style={{ fontFamily: 'var(--f-mono)', fontSize: 8, letterSpacing: '0.16em', textTransform: 'uppercase', opacity: 0.4, marginBottom: 4 }}>
              runwayfyi · admin
            </p>
            <h1 style={{ fontFamily: 'var(--f-display)', fontSize: 22, fontWeight: 700, letterSpacing: '-0.02em', marginBottom: 14 }}>
              Looks Editor
            </h1>
            <input
              value={search}
              ...
              style={{
                width: '100%', fontFamily: 'var(--f-mono)', fontSize: 10,
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {loadingShows ? (
              ...
              filteredShows.map((show) => (
                <button
                  key={show.id}
                  onClick={() => loadLooks(show)}
                  style={{
                    width: '100%', textAlign: 'left', padding: '10px 20px',
                    background: selectedShow?.id === show.id ? 'rgba(255,255,255,0.12)' : 'none',
                    border: 'none', borderBottom: '1px solid rgba(255,255,255,0.06)',
                    cursor: 'pointer', color: '#fff',
                  }}
                >
                  <div style={{ fontFamily: 'var(--f-display)', fontSize: 14, fontWeight: 700, letterSpacing: '-0.01em' }}>
                    {show.brand}
                  </div>
                  <div style={{ fontFamily: 'var(--f-mono)', fontSize: 8, letterSpacing: '0.1em', textTransform: 'uppercase', opacity: 0.4, marginTop: 2 }}>
                    {show.city} · {show.total_looks} looks
                  </div>
                </button>
              ))
            )}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--cream)' }}>
          ...
          {!selectedShow ? (
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12 }}>
              <p style={{ fontFamily: 'var(--f-display)', fontSize: 28, fontWeight: 700, letterSpacing: '-0.02em', color: 'var(--light)' }}>
                Select a show
              </p>
              <p style={{ fontFamily: 'var(--f-mono)', fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--light)', opacity: 0.6 }}>
            ...
            <>
              ...
              <div style={{
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontFamily: 'var(--f-mono)', fontSize: 8, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--light)', marginBottom: 2 }}>
                    {selectedShow.city} · {selectedShow.season} · {looks.length} looks
                  </p>
                ...
                <button
                  onClick={() => { setAddingNew(true); setBulkMode(false) }}
                  style={{
                    fontFamily: 'var(--f-mono)', fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase',
                    padding: '8px 14px', background: 'var(--ink)', color: '#fff', border: 'none', cursor: 'pointer',
                  }}
                >
                  + Add Look
                </button>
                <button
                  onClick={() => { setBulkMode(true); setAddingNew(false) }}
                  style={{
                    fontFamily: 'var(--f-mono)', fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase',
                    padding: '8px 14px', background: 'none', color: 'var(--mid)', border: '1px solid var(--bd)', cursor: 'pointer',
                  }}
                >
                  Bulk Add
                </button>
              ...
              {/* Add single look */}
              {addingNew && (
                <div style={{ padding: '14px 24px', background: 'var(--warm)', borderBottom: '1px solid var(--bd)', display: 'flex', gap: 8, flexShrink: 0 }}>
                  <input
                    value={newUrl}
                    ...
                    autoFocus
                    style={{ flex: 1, fontFamily: 'var(--f-mono)', fontSize: 10, padding: '8px 12px', border: '1px solid var(--bd)', background: '#fff', outline: 'none', color: 'var(--ink)' }}
                  />
                  <button onClick={handleAddLook} disabled={saving === -1} style={{ fontFamily: 'var(--f-mono)', fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '8px 16px', background: 'var(--ink)', color: '#fff', border: 'none', cursor: 'pointer' }}>
                    {saving === -1 ? 'Adding...' : 'Add'}
                  </button>
              ...
              {/* Bulk add */}
              {bulkMode && (
                <div style={{ padding: '14px 24px', background: 'var(--warm)', borderBottom: '1px solid var(--bd)', flexShrink: 0 }}>
                  <p style={{ fontFamily: 'var(--f-mono)', fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--light)', marginBottom: 8 }}>
                    Paste one URL per line — each will be added as a new look
                  </p>
                  <textarea
                    value={bulkUrls}
                    ...
                    rows={5}
                    autoFocus
                    style={{ width: '100%', fontFamily: 'var(--f-mono)', fontSize: 9, padding: '8px 12px', border: '1px solid var(--bd)', background: '#fff', outline: 'none', color: 'var(--ink)', resize: 'vertical', marginBottom: 8 }}
                  />
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={handleBulkAdd} disabled={bulkSaving} style={{ fontFamily: 'var(--f-mono)', fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '8px 16px', background: 'var(--ink)', color: '#fff', border: 'none', cursor: 'pointer' }}>
                      {bulkSaving ? 'Adding...' : `Add ${bulkUrls.split('\n').filter((u) => u.trim()).length} Looks`}
                    </button>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: 1, background: 'var(--bd)' }}>
                {loadingLooks ? (
                  <div style={{ padding: '60px', textAlign: 'center', fontFamily: 'var(--f-mono)', fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--light)', background: 'var(--cream)' }}>
                    Loading looks...
                  </div>
                ) : looks.length === 0 ? (
                  <div style={{ padding: '60px', textAlign: 'center', background: 'var(--cream)' }}>
                    ...
                    <p style={{ fontFamily: 'var(--f-mono)', fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--light)', opacity: 0.6 }}>Use "Add Look" or "Bulk Add" to get started</p>
                  </div>
                ) : (
                      ...
                      return (
                        <div key={look.id} style={{ background: 'var(--cream)', position: 'relative' }}>
                          ...
                          <div style={{ width: '100%', paddingBottom: '150%', position: 'relative', background: 'var(--warm)', overflow: 'hidden' }}>
                            ...
                            {/* Delete button */}
                            <button
                              onClick={() => handleDelete(look.id, selectedShow.id)}
                              disabled={!!isDeleting}
                              style={{
                                position: 'absolute', top: 6, right: 6,
                                background: 'rgba(192,57,43,0.85)', color: '#fff',
                                border: 'none', cursor: 'pointer', width: 22, height: 22,
                            ...
                            {isEditing ? (
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                <input
                                  ...
                                  onKeyDown={(e) => e.key === 'Enter' && handleSaveEdit(look.id)}
                                  autoFocus
                                  style={{ fontFamily: 'var(--f-mono)', fontSize: 8, padding: '5px 7px', border: '1px solid var(--ink)', background: '#fff', outline: 'none', color: 'var(--ink)', width: '100%' }}
                                />
                                <div style={{ display: 'flex', gap: 4 }}>
                                  <button onClick={() => handleSaveEdit(look.id)} disabled={!!isSaving} style={{ flex: 1, fontFamily: 'var(--f-mono)', fontSize: 8, letterSpacing: '0.08em', textTransform: 'uppercase', padding: '5px', background: 'var(--ink)', color: '#fff', border: 'none', cursor: 'pointer' }}>
                                    {isSaving ? '...' : 'Save'}
                                  </button>
                              ...
                              <button
                                onClick={() => { setEditingLook(look.id); setEditUrl(look.image_url || '') }}
                                style={{ width: '100%', fontFamily: 'var(--f-mono)', fontSize: 8, letterSpacing: '0.08em', textTransform: 'uppercase', padding: '5px', background: 'none', color: 'var(--light)', border: '1px solid var(--bd)', cursor: 'pointer' }}
                              >
                                Edit URL
                              </button>
                            )}
